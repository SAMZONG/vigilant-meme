...
- 🎯 Entry points to execute your program using `python -m <vigilant_meme>` or `$ vigilant_meme` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/SAMZONG/vigilant-meme/branch/main/graph/badge.svg?token=vigilant-meme_token_here)](https://codecov.io/gh/SAMZONG/vigilant-meme)
[![CI](https://github.com/SAMZONG/vigilant-meme/actions/workflows/main.yml/badge.svg)](https://github.com/SAMZONG/vigilant-meme/actions/workflows/main.yml)

Awesome vigilant_meme created by SAMZONG

## Install

from source
```bash
git clone https://github.com/SAMZONG/vigilant-meme vigilant_meme
cd vigilant_meme
make install
```
...
```bash
pip install vigilant_meme
```

## Executing

```bash
$ vigilant_meme run --port 8080
```

or

```bash
python -m vigilant_meme run --port 8080
```

or

```bash
$ uvicorn vigilant_meme:app
```
...
```bash
❯ vigilant_meme --help
Usage: vigilant_meme [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ vigilant_meme create-user --help
Usage: vigilant_meme create-user [OPTIONS] USERNAME PASSWORD
...
```bash
vigilant_meme create-user admin admin --superuser
```
...
Run with `vigilant_meme run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `vigilant_meme create-user`.
...
-----------------------------------------------------
vigilant_meme/__init__.py              4      0   100%
vigilant_meme/app.py                  16      1    94%
vigilant_meme/cli.py                  21      0   100%
vigilant_meme/config.py                5      0   100%
vigilant_meme/db.py                   10      0   100%
vigilant_meme/models/__init__.py       0      0   100%
vigilant_meme/models/content.py       47      1    98%
vigilant_meme/routes/__init__.py      11      0   100%
vigilant_meme/routes/content.py       52     25    52%
vigilant_meme/routes/security.py      15      1    93%
vigilant_meme/routes/user.py          52     26    50%
vigilant_meme/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from vigilant_meme.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in vigilant_meme/default.toml.

### As environment variables
```bash
export vigilant_meme_KEY=value
export vigilant_meme_KEY="@int 42"
export vigilant_meme_KEY="@jinja {{ this.db.uri }}"
export vigilant_meme_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
vigilant_meme_ENV=production vigilant_meme run
```
